...
function saveToLocalStorage() {
    ...
    localStorage.setItem('user', JSON.stringify({ isLoggedIn: user.isLoggedIn, username: user.username, email: user.email }));
}
...
function loginUser() {
    ...
    user.username = prompt("Enter your username:") || "User";
    user.email = prompt("Enter your email:") || "user@example.com";
    saveToLocalStorage();
    updateNavbar();
    ...
    if (user.isLoggedIn) {
        profileLink.innerHTML = `
            <div class="flex items-center space-x-2 cursor-pointer" onclick="showProfile()">
                <div class="profile-icon">${user.username[0].toUpperCase()}</div>
                <span class="text-white hover:text-blue-200 transition font-semibold">${user.username}</span>
            </div>
        `;
        signupLink.style.display = 'none'; // Hide Signup when logged in
    } else {
        profileLink.innerHTML = `<a href="#" class="text-white hover:text-blue-200 transition font-semibold block md:inline-block py-2 md:py-0" onclick="loginUser()">Login</a>`;
        signupLink.style.display = 'block'; // Show Signup when not logged in
    }
}
...
function showProfile() {
    ...
    const borrowedList = document.getElementById('profileBorrowedBooks');
    borrowedList.innerHTML = '';
    if (user.borrowedBooks.length === 0) {
        borrowedList.innerHTML = '<p class="text-gray-600">No books borrowed yet.</p>';
    } else {
        user.borrowedBooks.forEach(book => {
            borrowedList.innerHTML += `
                <li class="text-gray-800">${book.title} by ${book.author} (${book.genre})</li>
            `;
        });
    }
